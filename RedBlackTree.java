...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    RBTreeNode<T> buildRedBlackTree(BTree.Node<T> r) {
        ...
        boolean hasRight = r.getItemCount() == 2 ? true : false;
        boolean hasLeft = r.getItemCount() == 3 ? true : false;
        boolean hasMore = r.getItemCount() > 3 ? true : false;
        boolean hasChildren = r.getChildrenCount() != 0 ? true: false;
        if (hasRight) {
            RBTreeNode resultTree = new RBTreeNode(true, r.getItemAt(0));
            resultTree.right = new RBTreeNode(false, r.getItemAt(1));
            if (hasChildren) {
                resultTree.left = buildRedBlackTree(r.getChildAt(0));
                resultTree.right.right = buildRedBlackTree(r.getChildAt(2));
                resultTree.right.left = buildRedBlackTree(r.getChildAt(1));
            }
            return resultTree;
        } else if (hasLeft) {
            RBTreeNode resultTree = new RBTreeNode(true, r.getItemAt(1));
            resultTree.right = new RBTreeNode(false, r.getItemAt(2));
            resultTree.left = new RBTreeNode(false, r.getItemAt(0));
            if (hasChildren) {
                resultTree.right.right = buildRedBlackTree(r.getChildAt(3));
                resultTree.right.left = buildRedBlackTree(r.getChildAt(2));
                resultTree.left.right = buildRedBlackTree(r.getChildAt(1));
                resultTree.left.left = buildRedBlackTree(r.getChildAt(0));
            }
            return resultTree;
        } else if (hasMore) {
            RBTreeNode resultTree = new RBTreeNode(true, r.getItemAt(0));
            if (hasChildren) {
                resultTree.left = buildRedBlackTree(r.getChildAt(0));
                resultTree.right = buildRedBlackTree(r.getChildAt(1));
            }
            return resultTree;
        }
        return null;
